...
4) pool_rna_complement.py

All outputs are written into a new folder:
  Orthaganal_RNA_Pool_<N>nt
"""
...
Options:
  ...
  -O, --output   Final output filename for step 4.
                 Default: Orthaganal_RNA_Pool_<N>nt.txt
...
Output files inside that folder:
  ...
  Orthaganal_RNA_Pool_<N>nt.txt   (or filename from -O)
""".strip()
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Final output filename for step 4.",
    )
    parser.add_argument(
        "-U",
        "--User",
...
def main() -> None:
    ...
    out_dir = Path.cwd() / f"Orthaganal_RNA_Pool_{args.num_nt}nt"
    ...
    step4_final = Path(args.output).name if args.output else f"Orthaganal_RNA_Pool_{args.num_nt}nt.txt"
